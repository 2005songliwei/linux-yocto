					     ...
					     struct axienet_dma_q *q)
{
	...
	if (!q->eth_hasdre) {
		...
		for (i = 0; i < lp->tx_bd_num; i++)
			q->tx_buf[i] = &lp->tx_bufs[i * XAE_MAX_PKT_LEN];
	}
...
static void __axienet_device_reset(struct axienet_dma_q *q)
{
	u32 timeout;
